        ...
        <div className="p-8 min-h-screen">
            <div className="flex justify-between items-center mb-10">
                <div>
                    ...
                    <p className="text-muted-foreground">Edit public facing marketing pages.</p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {/* Merge fetched pages with defaults if they don't exist yet in DB */}
                    {defaultPages.map((defPage) => {
                        const dbPage = pages.find(p => p.id === defPage.id);
                        const title = dbPage?.hero?.title || defPage.hero.title;

                        return (
                            <Link key={defPage.id} href={`/admin/content/${defPage.id}`}>
                                <div className="group relative h-48 bg-[#121214] border border-white/5 rounded-3xl p-6 overflow-hidden hover:border-nyembo-sky/50 transition-all duration-300">
                                    ...
                                    <div className="relative z-10 flex flex-col h-full justify-between">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-muted-foreground">/{defPage.id}</p>
                                        </div>
                                    </div>
                                </div>
                            </Link>
